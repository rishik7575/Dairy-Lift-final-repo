...
const ProfileIcon = ({ className = '' }: ProfileIconProps) => {
  ...
  const handleProfileClick = () => {
    ...
    console.log('React Investor ProfileIcon clicked, attempting navigation...');

    try {
      // Try global navigation function first
      if ((window as any).navigateToProfile) {
        console.log('Using global investor navigation function');
        (window as any).navigateToProfile();
      } else {
        console.log('Global function not available, using direct investor navigation');
        window.location.href = '../profile.html';
      }
    } catch (error) {
      console.error('Navigation error in investor React component:', error);
      // Final fallback
      try {
        window.location.assign('../profile.html');
      } catch (fallbackError) {
        console.error('Fallback investor navigation also failed:', fallbackError);
      }
    }
  };
  ...
  const handleLoginClick = () => {
    ...
    console.log('Investor login button clicked, navigating to login.html');
